...
| `domains.ts` | No | `tests/domains.test.ts` |
...
| `alerts/noaa_tsunami.ts` | Yes (NOAA API) | No (requires live API) |
| `alerts/usgs_earthquake.ts` | Yes (USGS API) | No (requires live API) |
| `alerts/nws_weather.ts` | Yes (NWS API) | No (requires live API) |
| `api/middleware.ts` | No (pure logic) | No |
...
Run the full test suite: `bun test` (or `bun test tests/`).
